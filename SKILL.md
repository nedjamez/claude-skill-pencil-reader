...
> **New to Pencil MCP?** Pencil is a design tool whose files (`.pen`) can be read and edited programmatically via a local MCP server. The MCP exposes tools like `get_editor_state`, `batch_get`, and `batch_design`. See the [official Pencil MCP documentation](https://docs.pencil.dev/mcp) for setup instructions.
